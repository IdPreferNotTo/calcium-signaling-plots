...
if __name__ == "__main__":
    ...
    for i in range(1, 39):
        ...
        for ii in range(5000):
            shuffle(stationary_isis)
            var_1 = fc.k_corr(stationary_isis, stationary_isis, 1)
            rho_1s.append(var_1/var_0)
        ...
        ax1.hist(rho_1s, bins=25)
        ax1.axvline(p1, color = st.colors[5], label=f"Index = {i:d}")
        ax1.set_ylabel(r"$p(\rho_1)$")
        ax1.set_xlabel(r"$\rho_1$")
        ax1.set_xlim([-1, 1])
        ax1.legend(fancybox=False)
        # plt.savefig(home + f"/Plots/rho1_distribution/rho1_dist_{i:d}.png", dpi=300)
        # plt.show()
        plt.close()
    ...
    for dT, p1, err in zip(dTs, p1s, err_p1s):
        ax1.plot([dT, dT], [p1-err, p1 + err], color=st.colors[5], zorder=1)
    ...
    for ntr, p1, err in zip(ntrs, p1s, err_p1s):
        ax2.plot([ntr, ntr], [p1-err, p1 + err], color=st.colors[5], zorder=1)
