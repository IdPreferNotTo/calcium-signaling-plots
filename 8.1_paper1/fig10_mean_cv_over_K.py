...
if __name__ == "__main__":
    ...
    for i, K in enumerate(Ks):
        folder = home + "/Data/calcium_spikes_markov/Data_no_adap_K/"
        ...
        folder_langevin = home + "/Data/calcium_spikes_langevin_strat/Data_no_adap_K/"
    ...
    ax2.scatter(np.append(Ks[0:1], Ks[2:]), np.append(cv_isis[0:1], cv_isis[2:]), s=20, fc="w", ec=st.colors[0], zorder=1, label="Two-component")
    ...
    for i, K in enumerate(Ks):
        jsingle = j / K
        folder = home + "/Data/calcium_spikes_markov/Data_no_adap_pK/"
        ...
        folder_langevin = home + "/Data/calcium_spikes_langevin_strat/Data_no_adap_pK/"
    ...
    #jsingle = j / 10.
    #folder = home + "/Data/calcium_spikes_markov/Data_no_adap_pK/"
    #file_spikes = f"spike_times_markov_ip1.00_tau{tau:.2e}_j{jsingle:.2e}_K10_0.dat"
    #data_isis = np.loadtxt(folder + file_spikes)
    #mean = np.mean(data_isis)
    #cv_at_10 = np.std(data_isis) / mean

    #cv_over_K = []
    #for K in Ks:
    #    cv_at_K = cv_at_10 * np.sqrt(10./K)
    #    cv_over_K.append(cv_at_K)

    #ax4.plot(Ks, cv_over_K, c=st.colors[5], lw=10,  zorder=0)
    ...
    #ax1.plot(Ks, mean_isis, color=st.colors[2], zorder=1)
    #ax2.plot(Ks, cv_isis, color=st.colors[2], zorder=1)
    #ax1.scatter(Ks, mean_isis_langevin, s=20, fc="w", ec=st.colors[2])
    #ax1.text(0.95, 0.95, r"$p = {\rm const.}$", fontsize=11, transform=ax1.transAxes, ha="right", va='top')
    #ax2.scatter(Ks, cv_isis_langevin, s=20, fc="w", ec=st.colors[2])
    #ax2.legend(fancybox=False, fontsize=8)
    ...
    #ax3.plot(Ks, mean_isis, color=st.colors[2], zorder=1)
    #ax4.plot(Ks, cv_isis, color=st.colors[2], zorder=1)
    #ax3.scatter(Ks, mean_isis_langevin, s=20, fc="w", ec=st.colors[2])
    #ax3.text(0.95, 0.95, r"$pK = {\rm const.}$", fontsize=11, transform=ax3.transAxes, ha="right", va='top')
    #ax4.scatter(Ks, cv_isis_langevin, s=20, fc="w", ec=st.colors[2])
