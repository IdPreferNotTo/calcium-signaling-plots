...
if __name__ == "__main__":
    ...
    for IP3 in IP3s:
       ...
       if len(ISIs) == 0:
           rates.append(0)
           cvISIs.append(np.nan)
       else:
           meanISI = np.mean(ISIs)
           cvISI = np.std(ISIs)/meanISI
           meanISIs.append(meanISI)
           cvISIs.append(cvISI)
           rates.append(1/meanISI)

    IP3s_theory_rate = np.linspace(0.6, 2, 15)
    rates_theory = []
    for IP3_theory in IP3s_theory_rate:
        print(f"{IP3_theory:.2f}")
        file_r0_theory = home + f"/Data/Calcium/data/r0_ip{IP3_theory:.2f}_tau1.05e+01_j1.10e-02_N10.dat"
        rate = np.loadtxt(file_r0_theory)
        rates_theory.append(rate)

    IP3s_theory_cv = np.linspace(0.9, 2, 12)
    cvs_theory = []
    for IP3_theory in IP3s_theory_cv:
        print(f"{IP3_theory:.2f}")
        file_cv_theory = home + f"/Data/Calcium/data/cv2_ip{IP3_theory:.2f}_tau1.05e+01_j1.10e-02_N10.dat"
        cv = np.loadtxt(file_cv_theory)
        cvs_theory.append(cv)
