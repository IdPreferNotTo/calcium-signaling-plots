...
if __name__ == "__main__":
    ...
    for tau, p, tau_er, eps_er in parameters_it2:
        file_seq = home + f"/Data/calcium/markov/adaptive/fit/spike_times_markov_ip1.00_taua{tau_er:.2e}_ampa{eps_er:.2e}_tau{tau:.2e}_j{p:.2e}_K10_5.dat"
    ...
    print(np.mean(p1s_exp), np.std(p1s_exp)/np.sqrt(len(p1s_exp) -1))
    ...
    ax5.scatter(dTs_sim, p1s_sim, s=20, fc="w", ec=st.colors[0])
    ax5.scatter(dTs_exp, p1s_exp, s=20, fc="w", ec=st.colors[7], zorder=2)
    ax5.axhspan(-0.75, -0.5, color="C7", alpha=0.3, zorder=1)
    ...
    ax6.scatter(ntrs_sim, p1s_sim, s=20, fc="w", ec=st.colors[0])
    ax6.scatter(ntrs_exp, p1s_exp, s=20, fc="w", ec=st.colors[7], zorder=2)
    ax6.axhspan(-0.75, -0.5, color="C7", alpha=0.3, zorder=1)
