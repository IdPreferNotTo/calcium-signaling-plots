...
if __name__ == "__main__":
    ...
    for ax1, ax2, ax3, tau, p in zip([ax1, ax4], [ax2, ax5], [ax3, ax6], taus, ps):
        ...
        cbar_T_markov = fig.colorbar(cs_T_markov, ax=ax1)
        ...
        cbar_T_markov.ax.set_title(r"$\langle T \rangle$ / s", loc="center")
        ...
        cbar_cv_markov = fig.colorbar(cs_cv_markov, ax=ax2)
        ...
        cbar_cv_markov.ax.set_title(r"$C_V$", loc="center")
        ...
        cbar_dcv_markov = fig.colorbar(cs_dcv_markov, ax=ax3)
        ...
        cbar_dcv_markov.ax.set_title(r"$\delta C_V$", loc="center")
    ...
    for ax1, ax2, tau, p, tau_er, eps_er in zip([ax7, ax9, ax11], [ax8, ax10, ax12], [5, 5, 1], [0.015, 0.015, 0.06], [500, 500, 500], [0.2, 0.02, 0.1]):
        ...
        ax1.set_xticklabels([])
        ax1.set_ylabel(r"$p_{\rm ISI}(\hat{t})$")
        ax2.set_ylabel(r"$C_z(\hat{t})$")
        ...
        if tau == 5.:
            ...
            ax2.plot(ts/mean, corr/abs(corr[0]), color=st.colors[1])
        else:
            ax1.hist(data_isi/mean, density=True, fc=st.colors[3], bins=25, alpha=0.75)
            ax2.plot(ts/mean, corr/abs(corr[0]), color=st.colors[3])

    #plt.savefig(home + f"/Dropbox/LUKAS_BENJAMIN/RamLin22_2_BiophysJ/figures/fig4.pdf", dpi=300, transparent=True)
    plt.show()
