...
if __name__ == "__main__":
    ...
    i_effs = []
    n = len(data[0])
    with open(home + f"/Data/calcium_spikes_experimental/Spikes/HEK/HEK2/spike_times.dat", "w") as f:
        for j in range(1, n):
            ...
            if j not in [7, 10, 17, 19, 21, 22, 24, 25, 26]:
                f.write(f"{popt[2]:.2f} {popt[0]:.2f} {popt[1]:.2f} ")
                for ISI in ISIs[:-1]:
                    f.write(f"{ISI:.1f} ")
