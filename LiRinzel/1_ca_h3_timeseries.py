...
for j in range(10):
    ...
    for i in range(3):
        ...
        print(n_total, ip3)
        data = np.loadtxt(
            "/home/lukas/CLionProjects/li-rinzel-calcium-phd/out/lr_model_ca_waves_N{:d}_ip{:.2f}".format(n_total, ip3))
        t, ca, j1, n_open = np.transpose(data)
        t_max = 200
        maximum = fmin(ut.inverted_h3_nullcline, x0=0.1, args=(ip3,), full_output=True)
        print(maximum)
        ca_crit = maximum[0][0]
        h_crit = -maximum[1]
