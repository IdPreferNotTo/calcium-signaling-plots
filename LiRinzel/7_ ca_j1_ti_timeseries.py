...
for j in range(10):
    ...
    for i in range(2):
        ...
        axis[2*i +1].set_xlim(400,600)
        axis[2*i +1].plot(t, j1, lw=1, alpha=0.7, c="C2", label=r"$N_{{Channel}}=$ {:d}".format(n_total))
        axis[2*i +1 ].legend()
    plt.savefig("/home/lukas/Plots/lr_ca_phd/lr_j1_ip{:.2f}.pdf".format(ip3), transparent=True)
    plt.show()
    plt.close()