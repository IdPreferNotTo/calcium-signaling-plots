...
if __name__ == "__main__":
    ...
    ax1.text(0.10, 0.95, r"A$_1$", fontsize=11, transform=ax1.transAxes, va='top')
    ax2.text(0.10, 0.95, r"A$_2$", fontsize=11, transform=ax2.transAxes, va='top')
    ax3.text(0.10, 0.95, r"B$_1$", fontsize=11, transform=ax3.transAxes, va='top')
    ax4.text(0.10, 0.95, r"B$_2$", fontsize=11, transform=ax4.transAxes, va='top')
    ...
    for ax_mean, ax_cv, tau, j in zip(axis_mean, axis_cv, taus, js):
        ...
        cvs_langevin = []
        err_cvs = []
        folder_markov = home + "/Data/calcium/markov/renewal/fast_buffer/"
        folder_langevin = home + "/Data/calcium/langevin/renewal/fast_buffer/"

        file_spikes = f"spike_times_markov_buffer_bt0.00_ip1.00_tau{tau:.2e}_j{j:.2e}_K10_5.dat"
        data_isis = np.loadtxt(folder_markov + file_spikes)
        ...
        ax_cv.scatter(bTs, cvs, s=20, fc="w", ec=st.colors[0], zorder=2, label="Two-comp. \n + buffer")
    ...
    order = [0, 1, 2]
    ax2.legend([handles[idx] for idx in order], [labels[idx] for idx in order], fancybox=False, framealpha= 1.0, fontsize=7, loc=1)
    #plt.savefig(home + f"/Dropbox/LUKAS_BENJAMIN/RamLin22_1_BiophysJ/SUB2/figures/fig11.pdf", transparent=True)
    plt.show()