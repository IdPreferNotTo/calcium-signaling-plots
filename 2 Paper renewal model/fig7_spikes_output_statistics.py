...
if __name__ == "__main__":
    ...
    for k, (tau, j) in enumerate(zip(taus, js)):
        ...
        if k == 0:
            ...
            label = "B"
        ax1.text(0.05, 0.95, label + r"$_1$", fontsize=11, transform=ax1.transAxes, va='top')
        ax2.text(0.05, 0.95, label + r"$_2$", fontsize=11, transform=ax2.transAxes, va='top')
        ax3.text(0.05, 0.95, label + r"$_3$", fontsize=11, transform=ax3.transAxes, va='top')
        st.remove_top_right_axis([ax1, ax3, ax2])
        folder_markov_ip3 = home + "/Data/calcium/markov/renewal/over_ip3/"
        ...
        if k == 0:
            ax1.legend(fancybox=False, fontsize=7, loc=1, framealpha=1.0)
        ...
        ax3.plot(fs, spectrum_theory, color=color.palette[5], lw=1)
...
#plt.savefig(home + f"/Dropbox/LUKAS_BENJAMIN/RamLin22_1_BiophysJ/SUB2/figures/fig7.pdf")
plt.show()
