...
if __name__ == "__main__":
    ...
    ax_ts1.set_xlabel("$t$ / s")
    ax_ts1.set_xlim([0, 25])
    ax_ts1.set_ylabel(r"$J_{\rm puff}$")
    ax_ts1.set_ylim([0, 6])
    mean = np.mean(jpuffs)
    ax_ts1.plot(ts[plt_start:plt_stop], jpuffs[plt_start:plt_stop], c=st.colors[0])
    ax_ts1.axhline(N*mean_x, c=st.colors[2], label="$\mu = {:.2f}$".format(mean))
    ...
    ax_hist1.set_xlabel("$J_{\rm puff}$")
    ax_hist1.set_xlim([-1, 6])
    ax_hist1.set_ylabel(r"$P(J_{\rm puff})$")
    ax_hist1.plot(gauss001, fc.gaussian_dist(gauss001, mean, std01), c=st.colors[0])
    ax_hist1.hist(jpuffs_dt01, bins=50, alpha=0.7, color=st.colors[0], density=True,
    ...
    plt.savefig(home + "/Data/Calcium/Plots/puff_current_coarse_grain.png", transparent=True)
    plt.show()
