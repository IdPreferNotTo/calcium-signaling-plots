...
if __name__ == "__main__":
    ...
    jcas = np.logspace(-3, 0, 30)
    Ncl = 10
    Nch = 4

    tau = taus[17]  # [taus[30], taus[35], taus[40]][i]
    jca = jcas[15]  # [jcas[15], jcas[25], jcas[35]][i]
    print(tau, jca)
    ca_res = 0.33
    n_cl = 10
    n_ch = 4
    ...
    file = "ca_markov_ip1.00_tau{:.2e}_j{:.2e}_N10_0.dat".format(tau, jca)
    ...
    jpuffs_dt1 = coarse_grain_list(jpuffs_clear, coarse_grain_factor)
    ...
    ax.set_xlim([0, 1])
    ax.legend()
    plt.savefig(home + "/Data/Calcium/Plots/6_markov_jpuff_mean_var_over_ca_dynamic.pdf", transparent=True)
